...
class GlobalBuffer:
    def __init__(self, episode_capacity=configs.episode_capacity, local_buffer_capacity=configs.max_episode_length,
        ...
        self.obs_buf = np.zeros(
            ((local_buffer_capacity + 1) * episode_capacity, configs.max_num_agents, *configs.obs_shape), dtype=np.bool)
...
class Learner:
    def __init__(self, buffer: GlobalBuffer, summary: MySummary):
...
@ray.remote(num_cpus=1)
class Actor:
    def __init__(self, worker_id: int, epsilon: float, learner: Learner, buffer: GlobalBuffer, summary: MySummary):
        self.id = worker_id
        self.model = Network()
        self.model.eval()
    ...
    def run(self):
        ...
        while True:
            ...
            if done == False and self.env.steps < self.max_episode_length:
                ...
                if self.id == 0:
                    return_value = data[-1]
                    self.my_summary.add_float.remote(x=self.episode + 1, y=return_value, title="Return Value",
