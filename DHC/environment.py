...
class Environment:
    def __init__(self,num_agents: int = configs.init_env_settings[0], map_size: tuple = configs.map_size,map_length: int = configs.init_env_settings[1],
        ...
        if curriculum:
            ...
            # self.map_size = map_size todo:debug
    ...
    def observe(self):
        ...
        # # 获取行人x y 坐标 todo
        # coorlist = self.dynamic_ped.pde_df[self.t]
        # # 得到栅格化中的位置
        # dynamic_ped_map = self.dynamic_ped.get_pedcoor(coorlist)
        # self.t += self.t # 时间加一
        # # 静态地图和动态地图合并
        # self.map = self.map + dynamic_ped_map
        # # 将大于1的全部置位1
